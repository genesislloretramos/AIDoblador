...
def audio_segmentation_to_voice(
    ...
    # Fallback for Coqui XTTS if language is not supported
    if speakers_coqui:
        # Define supported languages for XTTS (same as in segments_coqui_tts)
        supported_lang_coqui = [
            "zh-cn", "en", "fr", "de", "it", "pt", "pl", "tr", "ru", "nl", "cs", "ar", "es", "hu", "ko", "ja"
        ]
        lang_code = fix_code_language(TRANSLATE_AUDIO_TO, syntax="coqui")
        
        if lang_code not in supported_lang_coqui:
            logger.warning(
                f"Language '{lang_code}' (requested as '{TRANSLATE_AUDIO_TO}') is not supported by Coqui XTTS. "
                f"Falling back to Edge TTS for speakers: {speakers_coqui}"
            )
            # Move speakers from Coqui to Edge
            speakers_edge.extend(speakers_coqui)
            speakers_coqui = []
