...
class MDX:
    ...
    def __init__(
        ...
        if "CPU" in self.provider[0]:
            sess_options.enable_cpu_mem_arena = False
            sess_options.execution_mode = ort.ExecutionMode.ORT_SEQUENTIAL
            sess_options.intra_op_num_threads = 1
            sess_options.inter_op_num_threads = 1
            
        self.ort = ort.InferenceSession(model_path, sess_options=sess_options, providers=self.provider)
    ...
    def process_wave(self, wave: np.array, mt_threads=1):
        ...
        # CPU Memory Protection: Use small chunks and sequential processing
        if self.device == torch.device("cpu"):
            # 10 seconds chunks for CPU to avoid OOM - extreme low memory mode
            chunk = 44100 * 10 
            waves = self.segment(wave, False, chunk)
            q = queue.Queue()
            
            # Process sequentially without threads
            for c, batch in enumerate(waves):
                mix_waves, pad, trim = self.pad_wave(batch)
                self.prog.total = len(waves) # Update progress bar total
        ...
        # Handle case where waves might have different length than processed_batches if error occurred
        if len(processed_batches) != len(waves):
             logger.error("Incomplete processed batches, please reduce batch size!")
             
        # Re-segment with original chunk size logic to match expected output structure
